...
def print_banner(config: Config):
    ...
    print("=" * 60)
    print()
    risk_per_trade = config.starting_capital * (config.risk_per_trade_pct / 100)
    print(f"  Capital:          ${config.starting_capital:,.2f}")
    print(f"  Risk per trade:   {config.risk_per_trade_pct}% (${risk_per_trade:.2f})")
    print(f"  Max position:     ${config.max_position_size_usd:.2f}")
    print(f"  Max portfolio risk: {config.max_portfolio_risk_pct}%")
    print(f"  Daily loss limit: ${config.max_daily_loss_usd:.2f} ({config.max_daily_loss_usd / config.starting_capital * 100:.0f}%)")
    print(f"  Stop-loss:        {config.stop_loss_pct}%")
    print(f"  Take-profit:      {config.take_profit_pct}% (R:R = 1:{config.take_profit_pct / config.stop_loss_pct:.1f})")
